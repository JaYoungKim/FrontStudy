...
## 9. 게시판 목록화면 만들기
-[Javascript] Json 데이터 추출연습을 먼저 진행합니다. 
https://drive.google.com/file/d/1ukdlkYSGyRljbYm2MqBJTf14VXeQgXF2/view?usp=sharing

Json 데이터를 불러와서 게시판 목록 화면을 만드세요 
table로 구현합니다. 
...
## 10. 게시판에 쿼리스트링 붙이기
게시판 목록화면을 가져온 후,
쿼리스트링 번호에 따라 노출되는 게시물 번호를 변경해주세요.

쿼리스트링 => 주소?page=1 형식으로 보이는 주소.
```
게시물이 50개일때 
주소#1 ->  50 ~ 41
주소#2 ->  40 ~ 31
...
쿼리스트링에 1~5가 아닐때, 기본 주소로 리다이렉트 
기본주소로 로딩될때, 50~40번 목록 보이기. 
```
Json 데이터가 늘어나면, 게시물 목록도 자동으로 변경되어야합니다. 
...
쿼리스트링에 1~6이 아닐때, 기본 주소로 리다이렉트 
기본주소로 로딩될때, 51~42번 목록 보이기. 
```



## 11. 페이지 네이션 만들기 
10번을 가져온 후 페이지네이션을 추가하세요. 
총 페이지 번호는 전체 게시물은 쿼리스트링 번호만큼 표기됩니다. 

한 페이지에 게시물이 몇개나 보이는가: 10개
페이지 번호는 몇개나 보이는가: 5개

각 페이지 번호를 클릭하면 쿼리스트링에 페이지 번호가 붙으며, 화면이 이동합니다. 
선택된 페이지는 []로 강조표기가 됩니다. 

```
처음 이전 [1] 2 3 4 5 다음 끝

//다음 클릭시 
처음 이전 [6] 7 8 9 10 다음 끝

//이전 클릭시
처음 이전 1 2 3 4 [5] 다음 끝

```
## 12. 프로토타입으로 탭 기능을 구현하세요.
